import {utils} from 'omoeo-helper/utilities'
...
export class apiCode {
  ...
  static getMessage(config = {}) {
    const {status_code = 500, fields, accepted_value, data, error_message, name, at, custom, format, map} = config
    ...
    if (!utils.isEmpty(map) && /%([\w]+)?%/.test(codeObj.description)) codeObj.description = utils.format(codeObj.description, map)
